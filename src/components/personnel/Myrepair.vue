        ...
        <el-card>
            ...
            <el-dialog title="接取报修单" :visible="editDialogVisible" width="50%" @close="editDialogClosed" :close-on-click-modal="false">
                <el-form :model="editForm" :rules="editFormRules" ref="editFormRef" label-width="100px">
                    ...
                    <el-form-item :label="editForm.status == 1 ? '接取时间' : '完成时间'">
                        <el-input v-model="editForm.updatetime" disabled></el-input>
                    </el-form-item>
                </el-form>
                <span slot="footer" class="dialog-footer">
                    <el-button @click="editDialogVisible = false">取 消</el-button>
                    <el-button type="primary" @click="editRepair" :disabled="disableBtn">完 成</el-button>
                </span>
            </el-dialog>
        </el-card>
    </div>
...
export default {
    ...
    methods:{
        ...
        async getPersonnelInfo(){
            ...
            this.queryInfo.personnelid = this.PerInfo.id;
        },
        async getRepairList(){
            const {data:res} = await this.$http.get('/repair/getRepairByPersonnelid',{
        ...
        async editRepair(){
            this.submitForm.status = 2;
            const {data:res} = await this.$http.get('/repair/updateRepairById',{
                params: this.submitForm
            })
            this.getRepairList();
            this.editDialogVisible = false;
        }
